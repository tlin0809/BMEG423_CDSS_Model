...
SEPSISdat_train$probSepsisLR <- predict(
  logReg,
  newdata = subset(SEPSISdat_train, select = -c(inhospital_mortality)),
  type = "link"                  # <<< CHANGED BACK: logit(p)
)
SEPSISdat_test$probSepsisLR <- predict(
  logReg,
  newdata = subset(SEPSISdat_test, select = -c(inhospital_mortality)),
  type = "link"                  # <<< CHANGED BACK
)
...
# save model ----
save(logReg, file=paste0(team,"_","LR 1.RData"))
