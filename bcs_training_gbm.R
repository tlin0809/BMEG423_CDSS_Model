...
dropFeats <- c("inhospital_mortality", 
               "travelmethod_adm", "traveldist_adm",
               "waterpure_adm", "cookloc_adm", 
               "lightfuel_adm", "tobacco_adm",
               "bednet_adm", "momagefirstpreg_adm", 
...
trainX <- as.matrix(SEPSISdat_train[, !(names(SEPSISdat_train) %in% dropFeats)])
testX  <- as.matrix(SEPSISdat_test[,  !(names(SEPSISdat_test)  %in% dropFeats)])

## Quick but not necessarily great way to find a threshold
SEPSISdat_train$probSepsisGBM <- predict(myTree,newdata= trainX)
SEPSISdat_test$probSepsisGBM <- predict(myTree,newdata= testX)
