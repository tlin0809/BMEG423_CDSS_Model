...
# Hyperparameter tuning part 1 - auto feature pruning
library(ranger)

rf_init <- ranger(
  as.factor(inhospital_mortality) ~ .,
...
## drop unimportant features based on impurity importance 
import <- rf_init$variable.importance
imp_sorted <- sort(import, decreasing = TRUE)

## keep top 80% of features (for now, ideally should find an optimal spot for this too)
keep_n <- round(length(imp_sorted) * 0.80)
imp_features <- names(imp_sorted)[1:keep_n]

cat("Number of important predictors kept: ", keep_n, "\n") # print number of predictors kept

SEPSIS_train_reduced <- SEPSISdat_train[, c("inhospital_mortality", imp_features)]
SEPSIS_test_reduced  <- SEPSISdat_test[,  c("inhospital_mortality", imp_features)]

# Hyperparameter tuning part 2 - grid search (on reduced features)
...
train_pred4 <- rf_model4$predictions[,2]
test_pred4  <- predict(rf_model4, SEPSIS_test_reduced)$predictions[,2]

roc_train4 <- roc(SEPSIS_train_reduced$inhospital_mortality, train_pred4)
roc_test4  <- roc(SEPSIS_test_reduced$inhospital_mortality,  test_pred4)

plot(roc_train4, main=paste0("RF Model 4 — Train AUC=", round(roc_train4$auc,3)))
plot(roc_test4, col="red", add=TRUE)
text(0.4,0.4,paste0("Test AUC=", round(roc_test4$auc,3)), col="red")
...
cat("Selected threshold:", as.numeric(threshold4), "\n")


# save model
saveRDS(rf_model4, paste0(team, "_RF_model.4.rds"))

myModel4 <- list(
  thresh = round(as.numeric(threshold4), 4),
  model_path = paste0(team, "_RF_model.4.rds"),
...
res <- rbind(
  res,
  evaluate_model(
    SEPSIS_train_reduced$inhospital_mortality,
    train_pred4,
    as.numeric(threshold4),
    "Training", 0
  )
)
...
res <- rbind(
  res,
  evaluate_model(
    SEPSIS_test_reduced$inhospital_mortality,
    test_pred4,
    as.numeric(threshold4),
    "Testing", 0
  )
